...
| 系统 | 说明 |
| --- | --- |
| 炉火大厅 | 米拉导师问候、职业称号、动量之火、今日推荐任务、三种能量模式 |
| 征程地图 | 三幕战役 + 战争迷雾：当前区域全开，下一区域剪影，远方藏雾 |
| 工坊（任务详情） | 叙事、目标、DoD、证据表单、AI Prompt、常见陷阱、当前禁止事项护栏 |
| Boss 之门 | 区域出口答辩：证据齐全 + 亲口讲清，才能进入下一区域 |
| 证据宝库 | 14 件神器（Common → Legendary），每件附 LinkedIn/博客/作品集/简历/面试建议 |
| 技能树 | 六棵树、证据驱动解锁：先用任务证明，再花技能点点亮 |
| 复习卡组 | 简化间隔重复（四档评分），任务完成后知识卡自动入组 |
| 秘境商店 | 金币购买可选的小型作品集项目，永不阻塞主线 |
| 深度冲刺 | 状态好时连推多关，结束生成 Session Recap |
| 篝火日志 | 冒险历史、反思、冲刺回顾、存档导出/导入 |

种子内容：**Unity Native Boundary Lab** 项目的 R0 + M0–M13 全部里程碑与 6 座 Boss 之门。
