...
| Boss 之门 | 区域出口答辩：证据齐全 + 五问亲口讲清才能过门；答辩全文存入日志作面试防线 |
| 证据宝库 | 18 件神器（Common → Legendary），每件附 LinkedIn/博客/作品集/简历/面试建议 |
...
种子内容：**Unity Native Boundary Lab** 项目的 R0 + M0–M13 全部里程碑、6 座 Boss 之门，以及可选的**生产经验营地**（Act 0 支线，B1–B6：把 5 年 Unity 生产经验变成显性证据）。基础弧线（R0–M3 + 初次信号之门）为深度可玩内容，M4+ 为锁定骨架，Act II/III 藏于迷雾。
