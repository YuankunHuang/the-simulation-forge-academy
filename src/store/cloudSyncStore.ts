...
export const useCloudSyncStore = create<CloudSyncStore>()(
  persist(
    (set, get) => ({
      ...
      disconnect: () => set({ passphrase: null, lastSyncedAt: null, lastError: null, status: "idle" }),
      ...
      pushNow: async () => {
        ...
        try {
          ...
          set({ status: "idle", lastSyncedAt: result.savedAt, lastError: null });
          return { ok: true };
        } catch (err) {
      ...
      pullNow: async () => {
        ...
        try {
          ...
          set({ status: "idle", lastSyncedAt: remote.savedAt ?? new Date().toISOString(), lastError: null });
          return { ok: true };
        } catch (err) {
          const message = err instanceof CloudSyncError ? err.message : "同步失败，请稍后再试。";
          set({ status: "error", lastError: message });
...
/**
 * 自动同步策略（在 App 根部挂载一次）：
 * - 启动时若本机是全新存档（无任何已完成任务）且已配置口令，静默尝试拉取一次云端存档，
 *   避免新设备开箱就是空的，同时不会覆盖已经在玩的进度。
...
export function useCloudAutoSync() {
  ...
  const triedInitialPull = useRef(false);

  // 启动时的谨慎自动拉取
  useEffect(() => {
    if (!passphrase || !autoSyncEnabled || triedInitialPull.current) return;
    triedInitialPull.current = true;
    const completedCount = Object.keys(usePlayerStore.getState().questCompletions).length;
    if (completedCount === 0) {
      void pullNow();
    }
